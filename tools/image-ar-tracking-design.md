...
| 验收项 | 目标 | 实测 | 结论 |
|---|---|---|---|
| 抖动（静止段 z 标准差） | 比裸 PnP 降 ≥50% | 0.04419 → 0.00972（One-Euro，mincutoff 0.3/beta 0.05） | **降 78%** ✅ |
| 滤波引入延迟 | ≤1 帧 | **+0 帧**（导数互相关口径） | ✅ |
| 稳态帧成本 | <33ms | 跟踪 9.9ms（KLT 3.2 + 单应 6.6）；检测 41.7ms 每 45 帧一次，摊薄后 ≈0.9ms/帧 | ✅（离线机器） |
| 出位姿 | — | 61/70，判定丢失 1 次 | ✅ |
| 角点误差 | — | 全程 3.31px，静止段 3.39px（卡宽约 250px → ≈1.3%） | ✅ |
