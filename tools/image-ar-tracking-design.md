...
| 验收项 | 目标 | 实测 | 结论 |
|---|---|---|---|
| 抖动（静止段 z 标准差） | 比裸 PnP 降 ≥50% | 0.05381 → 0.01129（One-Euro，mincutoff 0.3/beta 0.05） | **降 79%** ✅ |
| 滤波引入延迟 | ≤1 帧 | **+0 帧**（导数互相关口径） | ✅ |
| 稳态帧成本 | <33ms | 跟踪 11.1ms（KLT 3.2 + 单应 7.7）；检测 36ms 每 45 帧一次，摊薄后 ≈0.8ms/帧 | ✅（离线机器） |
| 出位姿 | — | 59/70，判定丢失 1 次（前 20 帧是"卡片很小 + 初始捕获"阶段） | ✅ |
| 角点误差 | — | 全程 8.14px，静止段 3.78px（卡宽约 250px → ≈1.5%） | ✅ |
